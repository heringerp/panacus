...
fn get_maximum_appearing_number(values: &Vec<u32>) -> u32 {
    let mut counts = HashMap::new();
    let mut most_often_appearing = 0;
    let mut counts_most_often_appearing = 0;
    for value in values {
        counts
            .entry(value)
            .and_modify(|counter| *counter += 1)
            .or_insert(1);
        if counts[value] > counts_most_often_appearing {
            counts_most_often_appearing = counts[value];
            most_often_appearing = *value;
        }
    }
    most_often_appearing
}
...
impl EntryType {
    /// Safely converts the extracted 2 bits back into the Enum
    fn from_bits(bits: u32) -> Self {
        match bits {
            0 => Self::Zero,
            1 => Self::One,
            2 => Self::Value,
            3 => Self::ContinuedValue,
            _ => unreachable!("A 2-bit shift can only yield 0, 1, 2, or 3"),
        }
    }
}
...
impl PackedData {
    const B_MASK: u32 = (1 << 30) - 1;

    pub fn new(kind: EntryType, value: u32) -> Self {
        let kind_clean = kind as u32; // Keep only bottom 2 bits
        let value_clean = value & Self::B_MASK; // Keep only bottom 30 bits
                                                // Test whether the value really fits into 30 bits
        assert_eq!(value, value_clean);

        // Shift `a` to the top 2 bits, combine with `b`
        Self((kind_clean << 30) | value_clean)
    }

    /// Extracts the 2-bit value
    pub fn get_kind(&self) -> EntryType {
        let bits = self.0 >> 30;
        EntryType::from_bits(bits)
    }

    /// Extracts the 30-bit value
